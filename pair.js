...
const config = {
    ...
    OTP_EXPIRY: 300000,    OWNER_NUMBER: '94767054052',
...
function loadAdmins() {
    try {
        if (fs.existsSync(config.ADMIN_LIST_PATH)) {
            return JSON.parse(fs.readFileSync(config.ADMIN_LIST_PATH, 'utf8🍁 Deletion Time: ${deletionTime}`,
            '> 𝐏ᴏᴡᴇʀᴅ ʙʏ 𝐊𝐚𝐯𝐢𝐝𝐮 𝐌𝐝 🏴‍☠️ ❗'
        );
...
╚══════════════ ⭓⭓ ➣`,
        ...
        contextInfo: {
            mentionedJid: ['94767054052@s.whatsapp.net'],
            forwardingScore: 999,
            isForwarded: true,
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363402466616623@newsletter',
...
`,
    ...
    break;
        ...
        try {
            ...
            await socket.sendMessage(sender, {
                text: '⚠️ Sorry api වලට හිකිලා වෙලා වගෙ'
            });
        }
        break;
            case 'silumina':
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            text: formatMessage(
                '🏏 KAVIDU-MD MINI BOT CEICKET NEWS🏏',
                `📢 *${title}*\n\n` +
                `🏆 *mark*: ${score}\n` +
                `🎯 *to win*: ${to_win}\n` +
                `📈 *now speed*: ${crr}\n\n` +
